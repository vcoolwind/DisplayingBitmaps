...
public class ImageCache {
    ...
    public static class ImageCacheParams {
        ...
        public void setMemCacheSizePercent(float percent) {
            ...
            // TODO 获取APP执行分配的内存 Runtime.getRuntime().maxMemory()
            Log.i(getClass().getSimpleName(),"maxMemory -- ["+Runtime.getRuntime().maxMemory()+"]");
    ...
    private static RetainFragment findOrCreateRetainFragment(FragmentManager fm) {
        ...
        if (mRetainFragment == null) {
            ...
            //TODO　commit()函数和commitAllowingStateLoss()函数的唯一区别就是当发生状态丢失的时候，后者不会抛出一个异常。
            // 通常你不应该使用这个函数，因为它意味可能发生状态丢失。当然，更好的解决方案是commit函数确保在Activity的状态保存之前调用，
            // 这样会有一个好的用户体验。除非状态丢失的可能无可避免，否则就不应该使用commitAllowingStateLoss()函数。
